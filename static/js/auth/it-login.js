/**
 * EduNaukri Auth — IT domain login (Job Seeker / Recruiter).
 * Role is determined by the server-rendered URL:
 *   /it/login/job-seeker/
 *   /it/login/recruiter/
 */
(function () {
  ...
  function submitCredentialsLogin() {
    ...
    fetch(loginEndpoint, fetchOptions)
      ...
      .finally(function () {
        if (window.location.href.indexOf("/it/login") !== -1) {
          isSubmitting = false;
  ...
  function startOAuth(provider) {
    ...
    var url = baseUrl + (baseUrl.indexOf("?") >= 0 ? "&" : "?") + "role=" + encodeURIComponent(selectedRole);
