(function () {
  ...
  function root() {
    return document.getElementById("rcdSettingsPage");
  }

  function csrf() {
    if (window.RCD_SETTINGS && window.RCD_SETTINGS.csrfToken) {
      return window.RCD_SETTINGS.csrfToken;
    }
  ...
  function setButtonLoading(button, loading) {
    if (!button) return;
    var label = button.querySelector(".rcd-set-btn__label");
    var spinner = button.querySelector(".rcd-set-btn__spinner");
  ...
  function clearPasswordErrors() {
    showFormError("rcdSetPasswordError", "");
    showFieldError("rcdSetCurrentPass", "");
    showFieldError("rcdSetNewPass", "");
    showFieldError("rcdSetConfirmPass", "");
  }
  ...
  function validatePasswordForm(current, newPass, confirm) {
    ...
    if (!current) {
      showFieldError("rcdSetCurrentPass", "Current password is required.");
      errors.push("current");
    }
    if (!newPass) {
      showFieldError("rcdSetNewPass", "New password is required.");
      errors.push("new");
    }
    if (!confirm) {
      showFieldError("rcdSetConfirmPass", "Please confirm your new password.");
      errors.push("confirm");
    }
    if (newPass && current && newPass === current) {
      showFieldError("rcdSetNewPass", "New password must be different from your current password.");
      errors.push("new");
    }
    if (newPass) {
      var rules = passwordRulesMet(newPass);
      if (!rules.length || !rules.upper || !rules.lower || !rules.digit || !rules.special) {
        showFieldError("rcdSetNewPass", "Password must meet all strength requirements below.");
        errors.push("new");
      }
    }
    if (newPass && confirm && newPass !== confirm) {
      showFieldError("rcdSetConfirmPass", "Passwords do not match.");
      errors.push("confirm");
    }
  ...
  function resetPasswordStrengthUI() {
    var bar = document.getElementById("rcdSetStrengthBar");
    var label = document.getElementById("rcdSetStrengthLabel");
    var rules = document.getElementById("rcdSetPassRules");
  ...
  function bindPasswordToggles() {
    document.querySelectorAll(".rcd-set-pass-toggle").forEach(function (btn) {
  ...
  function bindPasswordStrength() {
    var input = document.getElementById("rcdSetNewPass");
    var bar = document.getElementById("rcdSetStrengthBar");
    var label = document.getElementById("rcdSetStrengthLabel");
    var rules = document.getElementById("rcdSetPassRules");
    if (!input || !bar) return;
  ...
  function bindAccountForm() {
    var form = document.getElementById("rcdSetAccountForm");
    if (!form) return;
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var payload = {
        first_name: document.getElementById("rcdSetFirstName").value.trim(),
        last_name: document.getElementById("rcdSetLastName").value.trim(),
        phone: document.getElementById("rcdSetPhone").value.trim(),
      };
      var officialEmail = document.getElementById("rcdSetOfficialEmail");
      var designation = document.getElementById("rcdSetDesignation");
  ...
  function bindCompanyForm() {
    var form = document.getElementById("rcdSetCompanyForm");
    if (!form) return;
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var payload = {
        name: document.getElementById("rcdSetCompanyName").value.trim(),
        legal_name: document.getElementById("rcdSetCompanyLegalName").value.trim(),
        industry: document.getElementById("rcdSetCompanyIndustry").value.trim(),
        website_url: document.getElementById("rcdSetCompanyWebsite").value.trim(),
        description: document.getElementById("rcdSetCompanyDescription").value.trim(),
      };

      var btn = form.querySelector("button[type='submit']");
      if (btn) btn.disabled = true;

      patchJson(apiUrl("company"), payload)
        .then(function (body) {
          notify("success", body.message || "Company profile updated.");
        })
        .catch(function (err) {
          notify("error", err.message);
        })
  ...
  function bindPasswordForm() {
    var form = document.getElementById("rcdSetPasswordForm");
    var submitBtn = document.getElementById("rcdSetPasswordBtn");
    if (!form) return;

    ["rcdSetCurrentPass", "rcdSetNewPass", "rcdSetConfirmPass"].forEach(function (id) {
    ...
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var current = document.getElementById("rcdSetCurrentPass").value;
      var newPass = document.getElementById("rcdSetNewPass").value;
      var confirm = document.getElementById("rcdSetConfirmPass").value;
      ...
      postJson(url, {
        ...
        .catch(function (err) {
          ...
          if (msg.indexOf("Current password") >= 0) {
            showFieldError("rcdSetCurrentPass", msg);
          } else if (msg.indexOf("confirm") >= 0 || msg.indexOf("match") >= 0) {
            showFieldError("rcdSetConfirmPass", msg);
          } else if (msg.indexOf("Password") >= 0) {
            showFieldError("rcdSetNewPass", msg);
          } else {
            showFormError("rcdSetPasswordError", msg);
          }
          notify("error", msg);
        })
        .finally(function () {
  ...
  function bindRealtimeToggle(input, saveFn, options) {
    options = options || {};
    var row = input.closest(".rcd-set-toggle");
    if (!row) return;
    var busy = false;

    function persist() {
      if (busy) return;
      var key = input.getAttribute("data-notify-key");
      if (!key) return;
      busy = true;
      input.disabled = true;
  ...
  function bindConnected() {
    document.querySelectorAll(".rcd-set-disconnect").forEach(function (btn) {
      btn.addEventListener("click", function () {
  ...
  function renderActivityItems(items) {
    var list = document.getElementById("rcdSetActivityList");
    if (!list) return;
    list.innerHTML = "";
    ...
    items.forEach(function (event) {
      ...
      article.className = "rcd-set-audit__item";
  ...
  function bindSecurityActivity() {
    var refreshBtn = document.getElementById("rcdSetRefreshActivity");
  ...
  function bindSessions() {
    var revokeOthers = document.getElementById("rcdSetRevokeOthers");
    if (revokeOthers) {
    ...
    document.querySelectorAll(".rcd-set-revoke-session").forEach(function (btn) {
      btn.addEventListener("click", function () {
  ...
  function bindDeleteAccount() {
    var modalEl = document.getElementById("rcdSetDeleteModal");
    if (!modalEl) return;
    var modal = new bootstrap.Modal(modalEl);
    document.getElementById("rcdSetDeleteOpen").addEventListener("click", function () {
      document.getElementById("rcdSetDeletePass").value = "";
      document.getElementById("rcdSetDeleteError").hidden = true;
      modal.show();
    });
    document.getElementById("rcdSetDeleteConfirm").addEventListener("click", function () {
      var errEl = document.getElementById("rcdSetDeleteError");
      postJson(apiUrl("delete"), { password: document.getElementById("rcdSetDeletePass").value })
        .then(function (body) {
          modal.hide();
          notify("success", body.message);
          window.location.href = body.redirect_url || (window.RCD_SETTINGS && window.RCD_SETTINGS.loginUrl) || "/it/login/recruiter/";
        })
        .catch(function (err) {
  ...
  function bindNav() {
    document.querySelectorAll(".rcd-set-nav__link").forEach(function (link) {
      link.addEventListener("click", function () {
        document.querySelectorAll(".rcd-set-nav__link").forEach(function (l) {
