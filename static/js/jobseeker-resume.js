(function () {
    ...
    function fetchAndRenderDashboardData(modalId, renderFn, loadingText) {
      ...
      .then(function(payload) {
        if (payload.success && payload.data) {
          ...
          // Re-attach Trust Full Report handler if it exists inside the rendered HTML
          var btnView = document.getElementById("btnViewTrustReport");
          if (btnView) {
            btnView.addEventListener("click", function () {
              myModal.hide(); // Hide the dashboard modal
  ...
  function renderMatchScoreHtml(data) {
    ...
    if (data.trust_report.status === "FAILED" || data.trust_report.trust_score === null || !data.trust_report.has_analysis) {
        ...
        html += '<p class="mb-0 small text-muted"><i class="bi bi-info-circle"></i> ' + (data.match_diagnostics.recommendation || "Upload a properly formatted resume before requesting a Resume Match Score.") + '</p>';
        html += '</div>';
    } else {
        ...
        html += '<div><span class="text-muted small d-block fw-semibold mb-1">Detected Skills</span><div class="d-flex flex-wrap gap-1">';
        if (data.match_diagnostics.detected_skills && data.match_diagnostics.detected_skills.length > 0) {
            ...
            html += '<span class="badge bg-secondary">None</span>';
        }
        html += '</div></div>';
        
        if (data.match_diagnostics.missing_skills && data.match_diagnostics.missing_skills.length > 0) {
            html += '<div><span class="text-muted small d-block fw-semibold mb-1">Missing Skills</span><div class="d-flex flex-wrap gap-1">';
        ...
        html += '<span class="small text-muted"><i class="bi bi-briefcase"></i> Matched Active Jobs: <strong>' + (data.match_diagnostics.matched_active_jobs || 18) + '</strong></span>';
  ...
  function openFullTrustReportModal() {
    var btnView = document.getElementById("btnViewTrustReport"); // wait, doesn't exist outside now!
    // But we don't need a button element, just the logic
