"""Certificate upload, storage, and lifecycle management."""
...
from apps.it_recruitment.services.jobseeker_profile_manage_service import parse_date
...
class CertificateManagementService(BaseService):
    PROFILE_CONTRIBUTION = 6

    def __init__(self):
        self.repo = JobSeekerCertificationRepository()
        self.storage = StorageService()
        self.completion = JobSeekerProfileCompletionService()
    ...
    @BaseService.atomic
    def update(
        self, profile: JobSeekerProfile, cert_id, *, actor_id, data: dict
    ) -> JobSeekerCertification:
        cert = self._get_owned(profile, cert_id)
        payload = self._parse_payload(data, partial=True, existing=cert)
        cert = self.repo.update(cert, updated_by_id=actor_id, **payload)
        self._after_change(
            profile,
    ...
    @BaseService.atomic
    def replace_file(
        self, profile: JobSeekerProfile, cert_id, *, actor_id, uploaded_file
    ) -> JobSeekerCertification:
        cert = self._get_owned(profile, cert_id)
    ...
    @BaseService.atomic
    def delete(self, profile: JobSeekerProfile, cert_id, *, actor_id) -> None:
        cert = self._get_owned(profile, cert_id)
        name = cert.name
    ...
    def get_file(self, profile: JobSeekerProfile, cert_id) -> StoredFile:
        cert = self._get_owned(profile, cert_id)
    ...
    @staticmethod
    def resolve_status(cert: JobSeekerCertification) -> CertificateStatus:
        if cert.is_verified:
    ...
    def _upload_file(self, profile, uploaded_file, actor_id) -> StoredFile:
        try:
            return self.storage.upload(
                ...
                owner_type="job_seeker_certification",
                owner_id=profile.pk,
    ...
    def _get_owned(self, profile, cert_id) -> JobSeekerCertification:
        cert = (
    ...
    def _after_change(self, profile, *, event: str, title: str, body: str) -> None:
        ...
        JobRecommendationTriggerService().schedule_seeker_rebuild(
            profile.pk, reason=event, notify=True
        )
