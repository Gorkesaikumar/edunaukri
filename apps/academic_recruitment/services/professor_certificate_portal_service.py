"""Professor Certificate Management Center — dashboard and list context."""
...
class ProfessorCertificatePortalService(BaseService):
    def __init__(self):
        self.mgmt = ProfessorCertificateManagementService()
    ...
    def build(
        ...
        *,
        ...
        return CertificatePortalContext(
            summary=self._summary(all_certs),
            certificates=[self._map_card(c, pu) for c in page_obj.object_list],
            categories=list(CertificateCategory.choices),
            profile_contribution=ProfessorCertificateManagementService.PROFILE_CONTRIBUTION
            if all_certs
            else 0,
            ...
            upload_api_url=pu("professor_certificates_api"),
            list_api_url=pu("professor_certificates_api"),
        )

    def _summary(
        self, certs: list[ProfessorCertification]
        ...
        return [
            ...
            CertificateSummaryCard(
                ...
                f"+{ProfessorCertificateManagementService.PROFILE_CONTRIBUTION if certs else 0}%",
                "bi-pie-chart",
                "offer",
            ),
        ]

    def _map_card(self, cert: ProfessorCertification, pu) -> CertificateCard:
        ...
        return CertificateCard(
            ...
            preview_url=pu("professor_certificate_preview", certification_id=cert.pk)
            if has_file
            else None,
            download_url=pu("professor_certificate_download", certification_id=cert.pk)
            if has_file
            else None,
            detail_api_url=pu(
                "professor_certificate_detail_api", certification_id=cert.pk
            ),
